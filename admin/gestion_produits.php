...
if( isset($_POST['id_produit']) && isset($_POST['id_salle']) && isset($_POST['date_arrivee']) && isset($_POST['date_depart']) && isset($_POST['prix']) ) {
    ...
    // On change le format de date pour la BDD
    ...
    // Controle sur la durée minimum d'une réservation
    $timestamp_arrivee = strtotime($date_arrivee);
    $timestamp_depart = strtotime($date_depart);
    $datetime = new \DateTime($date_arrivee);
    $datetime->modify("+1 day");
    $str_datetime = $datetime->format("Y-m-d H:i:s");
    $timestamp_oneday = strtotime($str_datetime);

    if( $timestamp_depart < $timestamp_oneday ) {
        $msg .= '<div class="alert alert-danger mt-3">La durée minimum d\'une réservation est d\'une journée.</div>';
        $erreur = true;
    }
...
//--------------------------------------------------------------------------
$liste_produits = $pdo->query("SELECT * FROM salle, produit WHERE salle.id_salle = produit.id_salle ORDER BY id_produit");
...
?>
    ...
    <header class="container mt-5">
        <div class="pt-5 rounded col-12">
            <h1 class="mt-5">Gestion des produits</h1>
                    ...
                    <table class="table table-bordered bg-white">
                        ...
                        <?php
                            while($ligne = $liste_produits->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                // echo '<td>' . substr($ligne['date_arrivee'], 0, 10) . '</td>';
                                // echo '<td>' . substr($ligne['date_depart'], 0, 10) . '</td>';
                                ...
                                $calcul_arrivee = strtotime($ligne['date_arrivee']);
                                $calcul_depart = strtotime($ligne['date_depart']);
                                $duree = round(($calcul_depart - $calcul_arrivee)/60/60/24,0);
                                // Affichage du S de jour(s)
                                if ($duree == 1 || $duree == 0 || $duree == -1 ) {
                                    echo '<td>' . $duree . ' jour</td>';
                                } else {
                                    echo '<td>' . $duree . ' jours</td>';
                                }
